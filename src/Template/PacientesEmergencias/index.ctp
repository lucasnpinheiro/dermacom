                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($pacientesEmergencias as $pacientesEmergencia): ?>
                                <tr>
                                    <td><?= $pacientesEmergencia->has('paciente') ? $this->Html->link($pacientesEmergencia->paciente->id, ['controller' => 'Pacientes', 'action' => 'edit', $pacientesEmergencia->paciente->id]) : '' ?></td>
                                    <td><?= h($pacientesEmergencia->nome) ?></td>
                                    <td><?= $pacientesEmergencia->has('parentesco') ? $this->Html->link($pacientesEmergencia->parentesco->id, ['controller' => 'Parentescos', 'action' => 'edit', $pacientesEmergencia->parentesco->id]) : '' ?></td>
                                    <td><?= h($pacientesEmergencia->telefone) ?></td>
                                    <td><?= h($pacientesEmergencia->created) ?></td>
                                    <td><?= h($pacientesEmergencia->modified) ?></td>
                                    <td class="actions text-right">
                                        <?= $this->Html->link(null, ['action' => 'edit', $pacientesEmergencia->id], ['title' => __('Edit'), 'class' => 'btn btn-primary btn-sm btn-rounded fa fa-pencil']) ?>
                                        <?= $this->Form->postLink(null, ['action' => 'delete', $pacientesEmergencia->id], ['title' => __('Delete'), 'class' => 'btn btn-danger btn-sm btn-rounded fa fa-eraser', 'confirm' => __('Are you sure you want to delete # {0}?', $pacientesEmergencia->id)]) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>

                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($pacientesSoube as $pacientesSoube): ?>
                                <tr>
                                    <td><?= $pacientesSoube->has('paciente') ? $this->Html->link($pacientesSoube->paciente->id, ['controller' => 'Pacientes', 'action' => 'edit', $pacientesSoube->paciente->id]) : '' ?></td>
                                    <td><?= h($pacientesSoube->como) ?></td>
                                    <td><?= h($pacientesSoube->nome) ?></td>
                                    <td><?= h($pacientesSoube->telefone) ?></td>
                                    <td><?= $pacientesSoube->has('especialidade') ? $this->Html->link($pacientesSoube->especialidade->id, ['controller' => 'Especialidades', 'action' => 'edit', $pacientesSoube->especialidade->id]) : '' ?></td>
                                    <td><?= h($pacientesSoube->created) ?></td>
                                    <td><?= h($pacientesSoube->modified) ?></td>
                                    <td class="actions text-right">
                                        <?= $this->Html->link(null, ['action' => 'edit', $pacientesSoube->id], ['title' => __('Edit'), 'class' => 'btn btn-primary btn-sm btn-rounded fa fa-pencil']) ?>
                                        <?= $this->Form->postLink(null, ['action' => 'delete', $pacientesSoube->id], ['title' => __('Delete'), 'class' => 'btn btn-danger btn-sm btn-rounded fa fa-eraser', 'confirm' => __('Are you sure you want to delete # {0}?', $pacientesSoube->id)]) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>

                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($pacientesConvenios as $pacientesConvenio): ?>
                                <tr>
                                    <td><?= $pacientesConvenio->has('paciente') ? $this->Html->link($pacientesConvenio->paciente->id, ['controller' => 'Pacientes', 'action' => 'edit', $pacientesConvenio->paciente->id]) : '' ?></td>
                                    <td><?= $pacientesConvenio->has('convenio') ? $this->Html->link($pacientesConvenio->convenio->id, ['controller' => 'Convenios', 'action' => 'edit', $pacientesConvenio->convenio->id]) : '' ?></td>
                                    ...
                                    <td class="actions text-right">
                                        <?= $this->Html->link(null, ['action' => 'edit', $pacientesConvenio->id], ['title' => __('Edit'), 'class' => 'btn btn-primary btn-sm btn-rounded fa fa-pencil']) ?>
                                        <?= $this->Form->postLink(null, ['action' => 'delete', $pacientesConvenio->id], ['title' => __('Delete'), 'class' => 'btn btn-danger btn-sm btn-rounded fa fa-eraser', 'confirm' => __('Are you sure you want to delete # {0}?', $pacientesConvenio->id)]) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
